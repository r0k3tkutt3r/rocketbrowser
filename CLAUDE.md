...
**A tab IS a window.** Rocket uses native macOS window tabbing (Safari-style), not a custom tab bar. Every tab is an `NSWindow` owned by one `BrowserWindowController`, grouped by `window.tabbingIdentifier` (`rocket.browser`, or `rocket.private` for private windows so they never merge with normal ones). New tabs are created with `addTabbedWindow`. `AppDelegate` keeps every live controller in its `controllers` array (`register`/`unregister`, unregistered in `windowWillClose`) — that array is the only owner, so forgetting to register leaks nothing but makes the window vanish from app-level operations.
...
- History recording is skipped for private windows, non-http(s) schemes, and the navigation right after an error page (`suppressHistoryOnce`).
