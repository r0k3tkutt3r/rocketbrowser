...
- Settings are plain `UserDefaults` keys read with `object(forKey:) as? Bool ?? true` so the default is "on": `ShowBookmarksBar`, `BlockAds`, `HideCookieBanners`, `FingerprintProtection`, `SuggestionsEnabled`, plus `SuggestionsExcludedHosts`, `SuggestionsLastTrained`, `Homepage`.
